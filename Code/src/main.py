...
def train(model, loader):
    ...
    earlyStopping = 50  # stop training after this number of epochs without improvement
    while True:
        epoch += 1
        print('Epoch:', epoch)
...
def validate(model, loader):
    ...
    while loader.hasNext():
        ...
        for i in range(len(recognized)):
            rec=correct_sentence(recognized[i])
            dist = min(editdistance.eval(rec, batch.gtTexts[i]),editdistance.eval(recognized[i], batch.gtTexts[i]))
            numWordOK += 1 if (dist<=2)  else 0
            numWordTotal += 1
            numCharErr += dist
            numCharTotal += len(batch.gtTexts[i])
            print('[OK]' if dist <= 2 else '[ERR:%d]' % dist, ' ' , batch.gtTexts[i] , ' -> ',
                    recognized[i] , ' -> ', rec)
    ...
    print(f' Word accuracy: {wordAccuracy * 100.0}%. Character Accuracy rate: {100 - charErrorRate * 100.0}%.')
    return charErrorRate, wordAccuracy


def infer(model, fnImg):
    "recognize text in image provided by file path"
    img = cv2.imread(fnImg,cv2.IMREAD_GRAYSCALE) #preprocess(cv2.imread(fnImg, cv2.IMREAD_GRAYSCALE), Model.imgSize)
...
def infer_by_web(path, option):
    ...
    recognized,confidence = infer(model, path)
    recognise="| "+recognized+"  ( confidence : "+str(confidence*100)+"%)  wthout Correction  |  And  |  "+correct_sentence(recognized)+"  with correction |"
    return recognise
